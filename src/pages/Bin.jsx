...
import { getBinItems, searchBinItems } from "../services/BinService";
...
const Bin = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await getBinItems(currentPage, pageSize);
  ...
  const searchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await searchBinItems(currentPage, pageSize, searchParams.subject, searchParams.buyers, searchParams.startDate, searchParams.endDate);
      setData(response?.emails || []);
      setTotalPages(response?.pageable?.totalPages || 0);
    } catch (error) {
      console.error("Error fetching bin items:", error);
  ...
  const tableBody = useMemo(() => {
    ...
    return data.map((item) => (
      <tr key={item.id} className="border">
        ...
        <td className="p-2">{item.recieved_date}</td>
        <td className="p-2">{item.deleted_date}</td>
      </tr>
    ));
  ...
  return (
    <div className={`transition-all duration-300 p-6 bg-gray-100 min-h-screen`}>
      ...
      {/* Table */}
      <div className="border rounded-md shadow-md overflow-hidden bg-white mt-8">
