...
import { getDraftItems, searchDraftItems } from "../services/DraftService";
...
const Drafts = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await getDraftItems(pageSize, currentPage);
  ...
  const searchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await searchDraftItems(pageSize, currentPage, searchParams.subject, searchParams.buyers, searchParams.startDate, searchParams.endDate);
      setData(response?.emails || []);
      setTotalPages(response?.pageable?.totalPages || 0);
    } catch (error) {
      console.error("Error fetching draft items:", error);
    ...
    <div className={`transition-all duration-300 p-6 bg-gray-100 min-h-screen`}>
      ...
      <div className="grid grid-cols-6 gap-4 mb-4">
        ...
        <div className="flex flex-row justify-between col-span-2 me-6 h-11 mt-6">
          <button onClick={() => { searchData() }} className="flex items-center space-x-2 bg-blue-500 text-white px-4 py-2 rounded-md shadow-md">
            <FaSearch />
            <span>{t("weBuy")}</span>
          </button>
          <Button
